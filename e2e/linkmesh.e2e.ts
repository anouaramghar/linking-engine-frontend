...
test("an approved operator connects a site and navigates to the content pool", async ({
  ...
  await page.getByLabel("Connector").selectOption("html");
  ...
  await expect(
    page.getByText("Connect the first trusted RSS, Atom, or Wikipedia source."),
  ).toBeVisible();
...
test("an operator selects a suggestion, approves its exact edit, and queues publication", async ({
  ...
  await page.getByRole("link", { name: "Review selected links" }).click();
  ...
  await expect(page.getByRole("button", { name: "Approve and queue 1 article" })).toBeVisible();

  await page.getByRole("button", { name: "Approve and queue 1 article" }).click();
