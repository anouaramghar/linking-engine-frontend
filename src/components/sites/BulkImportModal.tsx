  ...
  return (
    ...
    >
      <div
        ...
        className="flex max-h-[85vh] w-full max-w-3xl flex-col rounded-2xl border border-stone-200 bg-stone-50 p-7"
      >
        <div className="mb-1 font-serif text-2xl">Import sites from CSV</div>
        ...
        {!result && (
          <label className="mb-4 cursor-pointer rounded-xl border border-dashed border-stone-300 bg-white px-4 py-6 text-center text-sm hover:border-stone-950">
            ...
            <span className="ml-2 text-stone-500">— click to browse</span>
          </label>
        )}

        {readError && <div className="mb-3 text-sm text-red-600">{readError}</div>}
                ...
                <tbody>
                  {parsed.rows.map((row) => (
                    <tr key={row.line} className="border-t border-stone-100">
                      <td className="px-3 py-2 tabular-nums text-stone-500">{row.line}</td>
                      {row.site ? (
                        <>
        ...
        {bulk.isError && (
          <div className="mt-3 text-sm text-red-600">
            {(bulk.error as { response?: { data?: { detail?: string } } }).response?.data?.detail ??
