...
export default function SitesPage() {
  ...
  const run = async (
    ...
    try {
      ...
      setJobs((current) => ({ ...current, [siteId]: job_id }));
      setNotice(`${label} job queued.`);
    } catch {
      setNotice(`${label} could not be queued. Please try again.`);
    }
  };
  ...
  return (
    <>
      ...
      <div className="relative overflow-y-auto px-8 py-6">
        ...
        <div
          className={`${GRID} px-5 pb-3 text-[11px] font-semibold uppercase tracking-widest text-stone-400`}
        >
          <div>Site</div>
          <div>Connector</div>
          <div>Added</div>
          ...
          {sites?.map((site, index) => (
            ...
            >
              ...
              <div className="flex items-center justify-end gap-1.5">
                <button
                  onClick={() => run(site.id, "Crawl", ingestSite)}
